...
def get_MSE_loss(original_volume: 'Volume', reconstructed_volume: 'Volume', empty_cached_memory: bool = True) -> float: # type: ignore  # noqa: F821
    ...
    result = float(cp.mean(cp.square(cp_reconstructed_volume - cp_original_volume)))
    
    if empty_cached_memory:
        del(cp_original_volume, cp_reconstructed_volume)
...
def empty_cached_gpu_memory() -> None:
    """Suppress the unused cache memory that cupy and pytorch are keeping for themselves."""
    gc.collect()
    cp.get_default_memory_pool().free_all_blocks()
