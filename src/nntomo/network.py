...
from nntomo.utilities import optimizer_to, get_MSE_loss, progressbar
...
def model_training(model: nn.Module, train_dataset: Dataset, valid_dataset: Dataset, device: str, batch_size: int, learning_rate: float, Nstop: int,
                   max_epoch: int, network_id: str, empty_cached_memory: bool) -> nn.Module:
    ...
    if empty_cached_memory:
        del(checkpoint, optimizer, model, loss_fn, train_dataloader, valid_dataloader, train_dataset, valid_dataset)
...
def nnfbp_training(train_dataset: DatasetNNFBP, valid_dataset: DatasetNNFBP, Nh: int = 8, batch_size: int = 32, learning_rate: float = 1e-4,
                   Nstop: int = 25, max_epoch: int = None, custom_id: str = None, empty_cached_memory: bool = True) -> NNFBP:
    ...
    return model_training(model, train_dataset, valid_dataset, 'cpu', batch_size, learning_rate, Nstop, max_epoch, network_id, empty_cached_memory)
...
def msdnet_training(train_dataset: DatasetMSDNET, valid_dataset: DatasetMSDNET, depth: int = 100, batch_size: int = 1, learning_rate: float = 1e-4,
                   Nstop: int = 25, max_epoch: int = None, custom_id: str = None, empty_cached_memory: bool = True) -> MSDNET:
    """Training procedure, with data saves every 30s. The training is done on CPU. Notably, The data saved contains an historic of the
    networks and losses during the training.
    ...
    return model_training(model, train_dataset, valid_dataset, 'cuda:0', batch_size, learning_rate, Nstop, max_epoch, network_id, empty_cached_memory)


def get_MSE_evolution(network_id: str, proj_stack: 'ProjectionStack', ref_volume: 'Volume', threshold: int = None) -> list[float]: # type: ignore  # noqa: F821
    ...
    Args:
        ...
        threshold (int, optional): A threshold for segmentation of the NN reconstuctions. Defaults to None (no thresholding).
    ...
    mse_list = []
    for i, state in progressbar(list(enumerate(checkpoint['model_states_history'])), "MSE computations:"):
